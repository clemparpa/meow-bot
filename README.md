...
To verify the stack boots on your laptop:
...
The signed-webhook → dispatch chain is exercised in-process by [`tests/integration/`](tests/integration/) (run via `uv run pytest`) — no docker required.
