...
- **Variables d'env consommées** : `GITHUB_WEBHOOK_SECRET`, `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY` *(inline PEM, optionnel)* / `GITHUB_APP_PRIVATE_KEY_PATH` *(fichier monté, défaut `/secrets/github-app.pem`)*, `MISTRAL_API_KEY`, `DEPLOYMENT_NAME`, `MEOW_BOT_LOGIN`, `KOYEB_API_TOKEN`.
